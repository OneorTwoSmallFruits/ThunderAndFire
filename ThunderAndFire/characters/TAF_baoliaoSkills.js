...
const { icefangcan, choosefangcan } = asyncs.qun.TAF_bl_shenjiaxu;//蝶贾诩
const { initicelianjie, initicejiangxian } = asyncs.qun.TAF_bl_Wuhuangfusong;//武皇甫嵩
...
const TAF_baoliaoSkills = {
    ...
    firelieqiong: {
        ...
        intro:{
            content: "players",
            onunmark:true,
            name:"<font color= #FF2400>裂穹·天冲</font>",
        },
        trigger: {
            global: ["damageEnd", "phaseAfter"],
        },
        async init(player, skill) {
        ...
        filter: function (event, player, name) {
            if (name == 'damageEnd') {
                return event.source && event.source === player && event.player && event.player.isAlive();
            } else {
                player.removeStorage('firelieqiong');
                player.unmarkSkill('firelieqiong');
                return;
            }
        },
        async content(event, trigger, player) {
            const halfnum = Math.ceil(trigger.player.getCards('h').length / 2);
            const list = [
                "〖力烽〗：令" + get.translation(trigger.player) + "随机弃置" + get.cnNumber(halfnum) + "张手牌。",
                "〖地机〗：令" + get.translation(trigger.player) + "下一次受到的伤害+1直到" + get.translation(trigger.player) + "回合结束。",
                "〖中枢〗：令" + get.translation(trigger.player) + "使用的下一张牌无效直到" + get.translation(trigger.player) + "回合结束。",
                "〖气海〗：令" + get.translation(trigger.player) + "不能使用或打出红桃牌直到" + get.translation(trigger.player) + "回合结束。",
                "〖天冲〗：令" + get.translation(trigger.player) + "失去剩余" + get.cnNumber(trigger.player.hp) + "点体力值，若" + get.translation(trigger.player) + "因此死亡，你+1体力上限。",
            ];
            let result = await player.chooseButton(["〖裂穹〗：请选择一项执行之！",
                [list.map((item, i) => {return [i, item];}),"textbutton",],
            ]).set("filterButton", function (button) {
                if (button.link === 0) { //力烽
                    return trigger.player.isAlive() && halfnum > 0;
                } else if (button.link === 1) { //地机
                    return trigger.player.isAlive() && !trigger.player.LQ_DJ;
                } else if (button.link === 2) {//中枢
                    return trigger.player.isAlive() && !trigger.player.LQ_ZS;
                } else if (button.link === 3) {//气海
                    return trigger.player.isAlive() && !trigger.player.LQ_QH;
                } else if (button.link === 4) {//天冲
                    return trigger.player.isAlive() && player.getStorage(event.name).includes(trigger.player);
                }
            }).set("selectButton", 1).set("ai", function (button) {
                    function getlinksNum () {
                        let links = [];
                        if (trigger.player.isAlive() && halfnum > 0) links.push(0);
                        if (trigger.player.isAlive() && !trigger.player.LQ_DJ) links.push(1);
                        if (trigger.player.isAlive() && !trigger.player.LQ_ZS) links.push(2);
                        if (trigger.player.isAlive() && !trigger.player.LQ_QH) links.push(3);
                        if (trigger.player.isAlive() && player.getStorage(event.name).includes(trigger.player)) links.push(4);
                        return links;
                    }
                    function getValueLinks() {
                        ...
                        if (att >= 2) return -1;
                        if (links.includes(4)) return 4;
                        if (links.includes(0)) {
                            if (halfnum >= 2) return 0;
                        }
                        if (links.includes(3)) return 3;
                    ...
                    switch (button.link) {
                        ...
                        case 3:
                            return getValueLinks() === 3;
                        case 4:
                            return getValueLinks() === 4;
                    }
            }).forResult();
            if (result.bool) {
                player.logSkill(event.name, trigger.player, 'fire');
                ...
                if (choices.includes(0)) {//力烽
                    trigger.player.LQ_LF = true;
                    const cards = trigger.player.getCards('h');
                    const halfnum = Math.ceil(trigger.player.getCards('h').length / 2);
                    if (halfnum > 0) await trigger.player.discard(cards.randomGets(halfnum), true);
                    if (!trigger.player.hasSkill('firelieqiong_JS')) trigger.player.addSkill('firelieqiong_JS');
                } else if (choices.includes(1)) {//地机
                    trigger.player.LQ_DJ = true;
                    if (!trigger.player.hasSkill('firelieqiong_JS')) trigger.player.addSkill('firelieqiong_JS');
                } else if (choices.includes(2)) {//中枢
                    trigger.player.LQ_ZS = true;
                    if (!trigger.player.hasSkill('firelieqiong_JS')) trigger.player.addSkill('firelieqiong_JS');
                } else if (choices.includes(3)) {//气海
                    trigger.player.LQ_QH = true;
                    if (!trigger.player.hasSkill('firelieqiong_JS')) trigger.player.addSkill('firelieqiong_JS');
                } else if (choices.includes(4)) {//天冲
                    await trigger.player.loseHp(trigger.player.hp);
                    if (game.getGlobalHistory("everything", evt => {
                        if (evt.name != "die" || evt.player != trigger.player) return false;
            ...
            effect: {
                player:function (card, player, target) {
                    ...
                    if (damage && get.attitude(player, target) < 0) {
                        if (get.attitude(player, target) <= 0 && player.getStorage('firelieqiong').includes(target)) return [target.hp, 1];
                    }
                },
            },
        },
            ...
            JS: {
                ...
                intro: {
                    content:function (storage, player) {
                        let result = "";
                        if (player.LQ_LF) result += "<font color= #FF2400>力烽</font>:已随机弃置手牌。<br>";
                        if (player.LQ_DJ) result += "<font color= #FF2400>地机</font>:下一次受到的伤害+1直到你的回合结束。<br>";
                        if (player.LQ_ZS) result += "<font color= #FF2400>中枢</font>:使用的下一张牌无效直到你的回合结束。<br>";
                        if (player.LQ_QH) result += "<font color= #FF2400>气海</font>：不能使用或打出红桃牌直到你的回合结束。"
                        return result;
                    },
                    onunmark: true,
                ...
                mod: {//气海
                    cardEnabled: function(card, player) {
                        if (get.suit(card)=="heart" && player.LQ_QH) return false;
                    },
                ...
                trigger: {
                    player: ["damageBegin", "useCard", "phaseEnd"],
                },
                charlotte: true,
                silent: true,
                unique: true,
                ...
                popup: false,
                async init(player, skill) {
                    const keys = ['LQ_LF', 'LQ_DJ', 'LQ_ZS', 'LQ_QH'];
                    for (let key of keys) {
                        if (!player[key]) player[key] = false;
                    }
                },
                filter: function (event, player, name) {
                    if (name === 'damageBegin') {//地机
                        return player.LQ_DJ && player.LQ_DJ === true && event.num > 0;
                    } else if (name === 'useCard') {//中枢
                        return player.LQ_ZS && player.LQ_ZS === true;
                ...
                async content(event, trigger, player) {
                    ...
                    if (Time === 'damageBegin') {//地机
                        trigger.num ++;
                        game.log(player,'因','#g【裂穹】','击中','#g【地机】',',本次受到的伤害+1。');
                        player.LQ_DJ = false;
                    } else if (Time === 'useCard') {//中枢
                        const targets =  trigger.targets;
                        if (targets && targets.length > 0) trigger.targets = [];
                        trigger.all_excluded = true;
                        game.log(player,'因','#g【裂穹】','击中','#g【中枢】，本次使用',trigger.card,'无效！');
                        player.LQ_ZS = false;
                        ...
                        player.removeSkill('firelieqiong_JS');
                        player.update();
                    }
                },
                sub: true,
    ...
    firezhanjue: {
        ...
        trigger: {
            player: ["phaseUseBegin", "useCard", "phaseUseEnd"],
            source: "damageAfter",
        },
        locked: false,
        direct: true,
        ...
        filter: function (event, player, name) {
            if (name === 'phaseUseBegin') {
                ...
                return event.card.name == "sha" && player.storage.firezhanjuedamage && player.isPhaseUsing();
            } else if (name === 'damageAfter') {
                return event.source === player && event.num > 0 && player.storage.firezhanjuerecover && player.isPhaseUsing();
        ...
        async content(event, trigger, player) {
            ...
            if (Time === 'phaseUseBegin') {
                ...
                let result = await player.chooseButton(["〖斩决〗：请选择一项执行之！",
                    ...
                    function getlinksNum () {
                        let links = [];
                        if (php > 0) links.push(0);
                        if (pdhp > 0) links.push(1);
                        return links;
                    }
                    function getValueLinks () {
                        let canuse = [];
                        ...
                        for (let card of damageCards) {
                            if(player.getCardsValue(card,false).length > 0) canuse.push(card);
                        }
                        const taos = player.getCards('hs').filter(card => get.name(card) === "tao" && player.canUse(card, player));
                        const links = getlinksNum();
                        if (links.includes(0)) {
                            if (php <= pdhp) {
                                if (taos.length > 0 && canuse.length > 0) return 0;
                                return 1;
                            } else {
                                return 0;
                            }
    ...
    iceluoshu:{
        ...
        direct:true,
        async init(player) {
            const luoshukey = lib.config.extension_银竹离火_TAFset_TAF_bl_shenjiaxu;//蝶贾诩络殊技能池拓展开关
