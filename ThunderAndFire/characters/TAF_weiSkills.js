...
const {
    thunderxingshangAI,thunderlvliAI,thunderquediAI,
} = setAI.wei;
...
const TAF_weiSkills = {
    ...
    thunderFZ03_fengyin:{
        ...
        skillBlocker:function (skill, player) {
            ...
            return skilllists.includes(skill) && !lib.skill[skill].charlotte;
        },
        mark:true,
        intro:{
            content:function (storage, player, skill) {
                const skilllists = player.thunderFZ03_fengyin.filter(item => {
                    return lib.skill['thunderFZ03_fengyin'].skillBlocker(item, player);
                });
    ...
    thundershanjia:{
        ...
        intro:{
            content:function(storage, player){
                ...
                if (!bool) {
                    ...
                    return setColor('阴：切换至本状态后，进入横置并获得技能〖御守〗，立即执行一次受到非⚡属性伤害内容，不计入次数限制');
                }
            },
        },
        trigger:{
        ...
        changeSkins: async function (player) {
            const names = ["thunder_caochun", "thunder_caochun1", "thunder_caochun2"];
            if (!changeSkinskey) return;
            const skinsID = player.checkSkins();
            if (!names.includes(skinsID)) return;
            if (skinsID === "thunder_caochun" || skinsID === "thunder_caochun1") {
                player.changeSkins(2);
            } else if (skinsID === "thunder_caochun2") {
            ...
            if (result.bool) {
                ...
                if (chooseCard.bool) {
                    ...
                    if (chooseCard.cards[0].original == "e" || chooseCard.cards[0].original == "h") {
                        ...
                        target.removeDisSkills(null, 'all');
                    }
                    if (player.countMark(event.name) >= 7) {
                        await player.tempDisSkills(event.name);
                    }
                }
            }
        },
    ...
    thunderquedi:{
        ...
        async content(event, trigger, player) {
            ...
            if (result.bool) {
                player.logSkill(event.name);
                player.tempDisSkills('thunderquedi');
    ...
    thunderchoujue:{
        ...
        async content(event, trigger, player) {
            ...
            player.recover();
            player.draw();
            player.removeDisSkills('thunderquedi');
        },
        "_priority":0,
    },
    //钟会
    thunderyulei:{
        ...
        trigger:{
            player:["damageBegin4","phaseUseBegin","phaseUseEnd"],
            source:["damageBegin4"],
        },
        locked:true,
        filter(event, player, name) {
            if (player.getExpansions('thunderyulei').length > 2) return false;
        ...
        async cost(event, trigger, player) {
            let count = trigger.num || 1;
            let exCardsNum = player.getExpansions('thunderyulei').length;
            const map = { cards: [], targets: [] };
            while (count > 0) {
                count--;
                await player.draw();
                let cards = player.getCards('he').filter(card => !map.cards.includes(card));
                if (cards.length === 0 || exCardsNum > 2) break;
                const prompt = setColor("〖御雷〗：请选择一张牌置于你的武将牌上作为⚡标记。");
                const result = await player.chooseCard(prompt, 'he', true, function(card) {
                ...
                if (result.bool) {
                    map.cards.push(result.cards[0]);
                    exCardsNum++;
                    if (exCardsNum > 2) {
                        const targets = game.filterPlayer(o => o.isIn() && o != player);
                        if (!targets || targets.length === 0) break;
                        const prompt = setColor("〖御雷〗：请选择一名其他角色与其均横置并弃置一张牌，然后本技能失效至本回合结束。");
                        const resultTarget = await player.chooseTarget(prompt, 1, function (card, player, target) {
                            return targets.includes(target);
                        }).set('ai', function (target) {
                            const friends = player.getFriends_sorted(false);
                        ...
                        if (resultTarget.bool) {
                            map.targets.push(resultTarget.targets[0]);
                        }
                        break;
                    }
                }
        ...
        async content(event, trigger, player) {
            ...
            if (targets && targets.length > 0) {
                ...
                player.tempDisSkills('thunderyulei');
            }
        },
        ai:{
            expose:0.25,
    ...
    thunderfulong:{
        ...
        locked:false,
        async init (player, skill) {
            if(!player.hasSkill('thunderfulong_clear')) player.addSkill('thunderfulong_clear');
        },
        filter(event, player, name) {
            if (!event.card || !event.source) return false;
            const cards = event.cards;
        ...
        shouyi(cards, player) { 
            ...
            for (const card of cards) {
                const num = getCardSuitNum(card) + getCardNameNum(card);
                numdraw += num;
            }
            const numdis = Math.floor(numdraw / 2);
            /**
             * 负面效果计算
             * 你将区域内牌数调整至三翻面横置并受到一点无来源的⚡伤害，然后本技能失效至该回合结束
             */
            let debuff = 0;
            const getdebuff = player.countMark('thunderfulong') + 1 - player.getDamagedHp();
            if (getdebuff > 0) {//即将达到负面效果
                const sum = player.getCards('he').length + numdraw - numdis;
        ...
        async cost(event, trigger, player) {
            const getShitiCards = trigger.cards;
            let numdraw = 0;
            for (const card of getShitiCards) {
                const num = getCardSuitNum(card) + getCardNameNum(card);
                numdraw += num;
            }
            const numdis = Math.floor((numdraw) / 2);
            const prompt = setColor("〖缚龙〗：是否弃置一张⚡标记：然后随机获得"+ get.cnNumber(numdraw) + "张点数不同且牌名不同的牌，并弃置"+ get.cnNumber(numdis) + "张牌？");
            let count = trigger.num || 1;
            let getcount = 0;
            while (count > 0) {
                count--;
                let cards = player.getExpansions('thunderyulei');
                if (cards.length === 0) break;
                const result = await player.chooseCardButton(cards, prompt).set('ai', function(button) {
                    return get.info('thunderfulong').shouyi(trigger.cards,player) >= 0;
                }).forResult();
                if (result.bool) {
                    player.addMark('thunderfulong', 1);
                    await player.discard(result.links)
                    getcount++;
                    if (player.countMark('thunderfulong') > player.getDamagedHp()) break;
                } else {
                    break;
                }
            }
        ...
        async content(event, trigger, player) {
            let { count, numdraw, numdis } = event.cost_data;
            while (count > 0) {
                count--;
            ...
            if (player.countMark('thunderfulong') > player.getDamagedHp()) {
                ...
                player.tempDisSkills('thunderfulong');
        ...
        ai:{
            ...
            effect:{
                target:function (card, player, target) {
                    const shouyi = get.info('thunderfulong').shouyi;
                    if (get.tag(card, "damage") && card.cards && card.cards.length > 0) {
                        if (!target.hasFriend()) return;
                        if (player.hasSkillTag("jueqing", false, target)) return [1, -2];
                        return [get.tag(card, "damage"), shouyi(card.cards,target) - get.tag(card, "damage")];
                    }
                    if (get.tag(card, "recover") && _status.currentPhase == target && !target.needsToDiscard()) {
                        const compareNum = Math.ceil(target.maxHp / 3);
                        if (target.hp >= compareNum) return [1, -2];
                    }
                },
                player:function (card, player, target) {
                    const shouyi = get.info('thunderfulong').shouyi;
                    const key1 = card.cards && card.cards.length > 0;
                    const key2 = get.tag(card, "damage") && get.attitude(player, target) < 2 && !player.hasSkillTag("jueqing", false, target);
                    if (key1 && key2) return [get.tag(card, "damage"), shouyi(card.cards, player)];
                },
            },
        },
        subSkill:{
    ...
    thunderyujun:{
        ...
        async init (player, skill) {
            ...
            if(!player.hasSkill('thunderyujun_clear')) player.addSkill('thunderyujun_clear');
        },
        filter:function(event, player, name) {
        ...
        async cost(event, trigger, player) {
            const cards = player.getExpansions('thunderyulei');
            if(!cards || cards.length == 0) return;
            const target = trigger.player;
            const prompt = setColor("〖与君〗：每轮游戏限一次，是否要选择一张⚡标记并横置，然后令" + get.translation(target) + "获得此牌并展示之？");
            const chooseButton = await player.chooseCardButton(cards,prompt, 1).set("ai", function (button) {
                const sorted = cards.sort((a, b) => {
                    const aValue = get.value(a, target);
                    const bValue = get.value(b, target);
                    if (bValue && aValue && aValue !== bValue) return bValue - aValue;
                    else return getCardNameNum(b) - getCardNameNum(a);
                });
                function useSkill() {
                    if (player.identity == "zhong" && target.identity == "zhu") return get.recoverEffect(target,player,player) > 0;
                    const att = get.attitude (player, target);
                ...
                if (useSkill()) {
                    ...
                    if(top2.includes(button.link)) return 1;
                    else return 0;
                } else {
                    return false;
                }
            ...
            if (chooseButton.bool) {
                ...
                player.$fullscreenpop("与君", "fire");
                const cards = chooseButton.links;
        ...
        async content(event, trigger, player) {
            ...
            const jiangweis = game.filterPlayer(o => {
                return lib.translate[o.name].includes("姜维") && o !== player && o !== trigger.player && o.isAlive();
            });
            if (jiangweis && jiangweis.length > 0) {
                lib.translate['thunderyujun_fettersSkill'] = "与君·羁绊";
                const next = game.createEvent('thunderyujun_fettersSkill');
                next.player = player;
                next.targets = jiangweis;
                next.setContent(async function (event, trigger, player) {
                    const targets = event.targets;
                    player.logSkill('thunderyujun', targets, 'thunder');
                    for (const target of targets) {
        ...
        subSkill:{
            clear: {
                ...
                async content(event, trigger, player) {
                    if(player.checkSkins() === 'thunder_zhonghui1') {
                        const chat = [
                            "八骏失驰，冰棱裂地，多少年功业，怎肯付西风！",
                            "天地不仁，神祇不恤，众生不余！",
                            "复仇，清算，即将开始！",
                            "一夕血海洗深仇，再立白骨做龙门！",
                        ];
                        const num = Math.floor(Math.random() * chat.length) + 1;
                        player.chat(chat[num - 1]); 
                        game.playAudio('..', 'extension', '银竹离火/audio/effect/thunder_zhonghui', 'thunderyujun_effect' + num);
                    }
