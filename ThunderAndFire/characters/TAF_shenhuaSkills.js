...
const { changeCharacter, initfanzhuan } = asyncs.qun.TAF_shen_tongyu;
const { shenwangjingtu } = asyncs.qun.TAF_shen_jiangtaixu;
const { setxuanyu } = asyncs.shu.TAF_shen_pangtong;
const { setlonghun, initlonghun } = asyncs.shu.TAF_shen_zhaoyun;
            ...
            if (chooseButton.bool) {
                ...
                if (choices.includes(0)) {
                    ...
                    const result = await player.chooseControl(lists).set('prompt', prompt).set ("ai", () => {
                        if (lists.includes('TAFxuanyu') && (!player.TAFxuanyu?.characters?.length)) return 'TAFxuanyu';
                        const viewAsSkills = lists.filter(skill => 
                            lib.skill.TAFxuanyu.filterSkills(skill, "viewAs").includes(skill)
                        );
                        const effectSkills = lists.filter(skill => {
                            const effect = lib.skill[skill]?.ai?.effect;
                            return effect && (typeof effect.player === 'function' || typeof effect.target === 'function');
                        });
                        const maixueSkills = lists.filter(skill => {
                            const ai = lib.skill[skill]?.ai || {};
                            return ai.maixie_defend || ai.maixie || ai.maixie_hp;
                        });
                        const otherSkills = lists.filter(skill => 
                            !viewAsSkills.includes(skill) && 
                            !effectSkills.includes(skill) && 
                            !maixueSkills.includes(skill)
                        );
                        if (otherSkills.length > 0) return otherSkills.randomGet();
                        if (maixueSkills.length > 0) return maixueSkills.randomGet();
                        if (effectSkills.length > 0) return effectSkills.randomGet();
                        if (viewAsSkills.length > 0) return viewAsSkills.randomGet();
                        return lists.randomGet();
                    }).set("forced", true).forResult();
                ...
                return {
                    ...
                    check: function (card) {
                        ...
                        if (key) {
                            ...
                            if (getcards.length >= 2 && get.value(getcards[0], player) < 8 && get.value(getcards[1], player) < 8) {
                                if (selectedCards.length === 0) {
                                    return get.value(card, player) == get.value(getcards[0], player);
                                }
                                if (selectedCards.length === 1) {
                                    return get.value(card, player) == get.value(getcards[1], player);
                                }
                            ...
                            for (const suit of lists) {
                                if (sameSuit[suit].length >= 2 && get.value(sameSuit[suit][0], player) < 8 && get.value(sameSuit[suit][1], player) < 8) {
                                    if (selectedCards.length === 0) {
                                        return get.value(card, player) == get.value(sameSuit[suit][0], player);
                                    }
                                    if (selectedCards.length === 1) {
                                        return get.value(card, player) == get.value(sameSuit[suit][1], player);
                                    }
                                    if (selectedCards.length > 1) return 0;
                                }
                            }
    ...
    TAFxingpan: {
        ...
        subSkill: {
            change: {
                ...
                async content(event, trigger, player) {
                    await changeCharacter(player, "TAF_shen_tongyu_shadow", "TAFxingpan");
                    ///debugger;
                },
                sub: true,
                sourceSkill: "TAFxingqi",
        ...
        async content(event, trigger, player) {
            ...
            if (Time == "dying") {
                ...
                if (result.control !== 'cancel2') {
                    ...
                    await changeCharacter(player, "TAF_shen_tongyu", "TAFxingpan_shadow");
                }
            } else {
                const missingSuits = player.checkXingpan('suit');
