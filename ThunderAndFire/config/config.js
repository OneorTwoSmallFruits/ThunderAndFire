...
import { characterSort, characterSortTranslate } from "../characters/sort.js";
...
const 武将分栏 = Object.keys(characterSortTranslate).map(key => characterSortTranslate[key]).join("丨");
export const config = {
    ...
    TAFset_qianyan: {
        ...
        item: {
            "1": "更新内容",
            "2": "武将更新<br>　　新增将灵协战分栏，全模式禁用在扩展全局设置内进行设置；为本扩展武将添加前缀，因为不少小伙伴反馈因美化出现bug，应该有美化扩展有读取武将前缀的相关函数，我这边统一添加武将前缀！",
            ...
            "6": "扩展更新<br>　　新增乱斗关卡:身份模式，除自选外均为本扩展武将，建议佩戴将灵闯关。",
            "7": "扩展更新<br>　　本次更新，扩展模块架构重新调整，可能会出现BUG(结构调整赋值的地方未及时调整)。后续更新会将在此基础上进行优化。",
        }
    },
    // 扩展说明
    TAFset_shuoming: {
        ...
        item: {
            "1": "查看说明",
            "2": "武将强度：十周年阴间偏上，不喜直删，谢谢配合。",
            "3": "另外声明1：个人不太喜欢强命、一刀斩的乱搞的设定。",
            "4": "另外声明2：这只是一个卡牌为核心的游戏，本扩展每个武将各技能之间的关联性、可玩性拉满，请放心食用。",
            "5": "另外声明3：不要讲您那所谓的“一眼阴间”，您张口就来的那所谓的阴间数值，是可以调的！",
            "6": "单机AI：本武将包中的所有武将均可由 单机呆逼AI 托管操作，符合其全部技能设定的要求及使用思路，主打一个单机陪玩。",
        }
    },
    // 扩展说明
    TAFset_designer: {
        ...
        item: {
            "1": "文案设计者名单",
            "2": "（一）神话再临：<br>　　姜太虚、神庞统 - 作者：白露为霜；<br>　　瞳羽 - 作者：想去远方",
            "3": "（一）星河皓月：<br>　　司马徽 - 作者：绘声03；",
            "4": "（二）惊鸿玉蝶：<br>　　张琪瑛 - 作者：想去远方；",
            "5": "（三）其他武将：：<br>　　SE孙鲁班 - 作者：迭；<br>　　喵林夕、SE贾诩 - 作者：夜啼林夕；<br>　　躬耕南阳诸葛亮 - 作者：落梦萧萧；<br>　　明徐阶 - 作者：数学家；",
        }
    },
    // 运行环境&扩展问题
    TAFset_huanjing: {
        ...
        item: {
            "1": "运行环境",
            "2": "　　① 较新的游戏版本（1.10.17.3及以上必需，无名杀本体）；<br>　　② “十周年 UI”扩展（推荐）；<br>　　③“千幻聆音”扩展（推荐）。",
        }
    },
    ...
    TAFset_skinschange: {//设置3.4：武将局内皮肤切换设置
        name: "3.3　武将皮肤切换设置",
		init: true,
        intro: "<br>　　①扩展内：司马懿、张春华、郭嘉、诸葛亮、周瑜、曹纯、赵襄，等部分武将内置了一些皮肤，会在特定状态下进行皮肤切换；默认开启！",
    },
    TAFset_charReplace: {//设置3.5：加强贾诩
        name: "3.4　同名武将切换设置（隐忍至神话）",
		init: true, // 默认开启
        intro: "选将页面如果出现本扩展武将，将出现可以切换的同名的全扩所有武将(除禁将外)；默认开启！",
    },
    TAFset_lutou: {//设置3.6：武将露头皮肤设置
        name: "3.5　武将露头皮肤设置(未找露头皮肤资源，请见谅)",
    ...
    TAFset_SkillsTips: {//设置3.7：武将技能悬浮显示
        name: "3.6　武将技能悬浮显示",
		init: true,
        intro: "本扩展部分武将技能描述将进行悬浮显示，建议开启；默认开启。",
    },
    TAFset_TAF_bl_shenjiaxu: {//设置3.8：加强贾诩
        name: "3.7　蝶贾诩络殊技能池增加",
		init: false, // 默认关闭
    ...
    addThunderAndFireFavoriteCharacters: {//设置4.1：一键添加《武将收藏列表》
        ...
        onclick: function () {
            if (this.innerHTML === '<small><font color=\"red\">确认收藏</font></small>') {
                ...
                const that = this;
                setTimeout(() => {
                    that.innerHTML = '<small><font color= #00FFFF>4.1　一键添加《<font color= #EE9A00>武将收藏列表</font>》</font><font color= #EE9A00>丨</font>(<font color=\"red\">隐忍至神话</font>)</small>';
                }, 1500);
            }
        },
    },
    clearThunderAndFireFavoriteCharacters: {//设置4.2：一键清除《武将收藏列表》
        name: '4.2　一键清除《武将收藏列表》丨(仅限本扩展)',
        intro: "点击确认清除后:将一键移除《武将收藏列表中》本扩展的所有武将！",
        clear: true,
        onclick: function () {
            if (this.innerHTML === '<small><font color=\"red\">确认清除</font></small>') {
                const charactersToRemove = Object.keys(characters);
                ...
                const that = this;
                setTimeout(() => {
                    that.innerHTML = '<small><font color= #00FFFF>4.2　一键清除《<font color= #EE9A00>武将收藏列表</font>》</font><font color= #EE9A00>丨</font>(<font color=\"red\">仅限本扩展</font>)</small>';
                }, 1500);
            }
        },
    },
    ...
    TAFset_addjianglingSkills: {
        name: "4.8　将灵协战",
        init: "0",
        intro: "重启生效，默认关闭，游戏开始时，令至多X名玩家各获得一位将灵协战(每名角色限一位，不可重复)；若有剩余将灵玩家可在出牌阶段自由替换剩余将灵,进行体验。",
        item: {
            "0": "关闭协战",
            "1": "至多一位",
            "2": "至多两位",
        ...
        if(item && typeof item === "object"){
            ...
            for (let key of keys) {
                ...
                const list1 = [
                    ...
                    '默认背景','自动切换','五秒','十秒','二十秒','半分钟','一分钟','两分钟','五分钟','络殊技能池','关闭协战',
                    '至多一位','至多两位','至多三位','至多四位','至多五位','至多六位','至多七位','至多八位','至多九位','至多十位',
                ];
                const list2 = [
